...
describe('timed out attempts', () => {
  ...
  const claimTimeout = async () =>
    await test.creatorClaimTimeout({
      ...
      expectedRefund: 4.416227, // initial prize + (1 - service fee)
      expectedRefundApproximation: 3,
      expectedServiceShare: 0, // service fee
      expectedServiceShareApproximation: 1,
      expectedGameTotalPuzzlesTimedOut: 1,
      expectedGameTotalPuzzlesTimedOutValue: 4.416227,
      expectedPlayerTotalPuzzlesTimedOut: 1,
      expectedPlayerTotalPuzzlesTimedOutValue: 4.416227,
    });
  ...
  beforeAll(async () => {
    ...
    await test.faucetToken(test.gbMint, test.creator1GbTokenAccount, 2);
    await test.faucetToken(test.gbMint, test.solver1GbTokenAccount, 10);
